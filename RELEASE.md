...
- The real Microsoft Defender, cloud protection included, in a disposable Windows 11 VM on QEMU/KVM (`scripts/defender-vm.sh`), run from Linux/WSL without anything on the Windows side. This is the only scan that produces Defender's cloud/ML verdicts. One-time setup: `scripts/defender-vm.sh create --iso <Windows 11 ISO>` installs Windows unattended and an OpenSSH server for host control, `prepare` updates the Defender platform, reboots so the platform switch is committed, updates the definitions and validates the cloud connection, `clean` freezes that disk as the baseline. Every `scan` boots a throwaway copy of the baseline, waits until the Defender service runs on the committed platform, copies the files in (real-time protection judges them on write), updates definitions, runs `MpCmdRun.exe` without remediation, collects detections, versions and the Defender event log, and discards the copy. The VM scan fails (it does not count as clean) when its definitions end up below the version the offline engine just downloaded. `make av-check` runs it automatically when the baseline exists (`~/.cache/defender-vm`, or `DEFENDER_VM_DIR`). Needs `qemu-system-x86_64` with `/dev/kvm`, `swtpm` and `xorrisofs`; the VM takes about 25 GB.
